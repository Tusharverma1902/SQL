...
from products
where price in (select MIN(price) from products group by category)
...
select *,ROW_NUMBER() over (order by price desc) as product_ranking 
from products)
...
select top 1 *
from (select top 2*
from products order by price desc) as top_2
order by price

-- with rank
select *
from(
select *, rank()over (order by price desc) as product_rank
from products) as ranking
where ranking.product_rank =2
